...
//	varLog |= 0x00000400;
	switch(unpackType){
		...
		case 0xC:
			skipmeminc = size;
#ifdef VIFUNPACKDEBUG
			SysPrintf("Processing V4-32 skip, size = %d, skip = %d\n", size, skipmeminc);
#endif
			break;
		case 0xD:
			skipmeminc = size * 2;
		...
		int offinc, destinc, unpacksize;
			...
			//while (size >= ft->dsize) {
				//if (vif->cl <= vifRegs->cycle.wl) { /* unpack one qword */
					...
					offinc = funcP(dest, (u32*)cdata, unpacksize);
...
//		}
	...
	if( size > 0  && vifRegs->num > 0) {
			...
			while (size >= ft->dsize) {
				//if (vif->cl <= vifRegs->cycle.wl) { /* unpack one qword */
					funcP(dest, (u32*)cdata, (size/ft->dsize));
					size = 0;
					//break;
				//}
				/*if (vif->cl == vifRegs->cycle.wl) {
	...
	if(vifRegs->num != 0 && v->size == 0)
		SysPrintf("Vifnum = %d\n", vifRegs->num);

if(skipped)skipmeminc += (((vifRegs->cycle.cl - vifRegs->cycle.wl)<<2) * 4) * skipped;
	/*if (size > 0) {

		SysPrintf("*PCSX2*: warning size(%d) > 0 after unpack\n", size);
...
int vif0transferData(u32 *data, int size) {
	...
	if ((vif0.cmd & 0x60) == 0x60) { // UNPACK
		...
		//	g_vifCycles+= size >> 1;
			vif0.tag.addr += skipmeminc;
			if(skipped){
#ifdef VIFUNPACKDEBUG
				SysPrintf("Caught it skipped Vif0\n");
#endif
			}
			vif0.tag.size -= size; 
			ret = size;
...
int VIF0transfer(u32 *data, int size, int istag) {
	...
	while (size > 0) {
		...
		if ((vif0.cmd & 0x80) && !(vif0Regs->err & 0x1) && (vif0.cmd & 0x7f) != 0x7) { //i bit on vifcode and not masked by VIF0_ERR
#ifdef VIF_LOG
	...
	if (vif0.irq > 0) {
		...
		//SysPrintf("Stall on vif0, FromSPR = %x, Vif1MADR = %x Sif0MADR = %x STADR = %x\n", psHu32(0x1000d010), vif0ch->madr, psHu32(0x1000c010), psHu32(DMAC_STADR));
		
		
		if(size > 0) {
			//SysPrintf("VIF0 Remaining size %x, data %x_%x_%x_%x\n", size, data[3], data[2], data[1], data[0]);
		}
		return -2;
	}
...
int vif1transferData(u32 *data, int size) {
	...
	if ((vif1.cmd & 0x60) == 0x60) { // UNPACK
		...
		//	g_vifCycles+= size >> 1;
			vif1.tag.addr += skipmeminc;
			if(skipped){
#ifdef VIFUNPACKDEBUG
				SysPrintf("Caught it skipped Vif1\n");
#endif
			}
			vif1.tag.size -= size; 
			ret = size;
...
int VIF1transfer(u32 *data, int size, int istag) {
	...
	while (size > 0) {
		...
		if ((vif1.cmd & 0x80) && !(vif1Regs->err & 0x1) && (vif1.cmd & 0x7f) != 0x7 && (psHu32(DMAC_CTRL) & 0xC) == 0x0) { //i bit on vifcode and not masked by VIF1_ERR
#ifdef VIF_LOG
...
int _vif1Interrupt() {
	...
	if(vif1.vifstalled == 1) {
		...
		if((vif1ch->chcr & 0x70000000) != 0x70000000)return 1;
	}
