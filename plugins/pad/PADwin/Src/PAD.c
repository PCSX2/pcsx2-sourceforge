...
u8   PADpollDigital(u8 value) {
	...
	if (curByte == 0) {
		...
		curCmd = value;
		stdpar[2] = status[curPad-1] >> 8;
		stdpar[3] = status[curPad-1] & 0xff;
		cmdLen = 4;
		buf = stdpar;
		return 0x41;
	}
...
u8  _PADpoll(u8 value) {
	...
	if (curByte == 0) {
		...
		switch (value) {
			...
			case 0x42: // READ_DATA
				if(lanalog.button) status[curPad] &= ~(1<<9);
				else status[curPad] |= (1<<9);
				if(ranalog.button) status[curPad] &= ~(1<<10);
				else status[curPad] |= (1<<10);

				lanalog.button = 0; // reset them :p (l3)
				ranalog.button = 0; // reset them :P (r3)

				stdpar[curPad][2] = status[curPad] >> 8;
				stdpar[curPad][3] = status[curPad] & 0xff;
				stdpar[curPad][4] = ranalog.x;
				stdpar[curPad][5] = ranalog.y;
				stdpar[curPad][6] = lanalog.x;
				stdpar[curPad][7] = lanalog.y;
...
u8   CALLBACK PADpoll(u8 value) {
	u8 ret;
/*	if (conf.analog) {
		ret = PADpollAnalog(value);
	} else {
		ret = PADpollDigital(value);
	}*/
	ret = _PADpoll(value);
#ifdef PAD_LOG
...
/*
u32  CALLBACK PADgetType() {
	if (conf.analog) return PAD_TYPE_ANALOG;
	else return PAD_TYPE_DIGITAL;
}*/
...
keyEvent* CALLBACK PADkeyEvent() {
	...
	if (event.event != 0) {
		memcpy(&ev, &event, sizeof(keyEvent));
		event.event = 0;
		return &ev;
	}
