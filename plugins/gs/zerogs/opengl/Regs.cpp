...
{
	tex1Info& tex1 = ZeroGS::vb[0].tex1;

	if( conf.bilinear && (tex1.mmag != ((data[0] >>  5) & 0x1) || tex1.mmin != ((data[0] >>  6) & 0x7)) ) {
		ZeroGS::Flush(0);
		ZeroGS::vb[0].bVarsTexSync = FALSE;
	}
	tex1.lcm  = (data[0]      ) & 0x1; 
...
{
	tex1Info& tex1 = ZeroGS::vb[1].tex1;

	if( conf.bilinear && (tex1.mmag != ((data[0] >>  5) & 0x1) || tex1.mmin != ((data[0] >>  6) & 0x7)) ) {
		ZeroGS::Flush(1);
		ZeroGS::vb[1].bVarsTexSync = FALSE;
	}
...
{
    int fba = *data & 0x1;
    if( ZeroGS::vb[0].fba.fba != fba ) {
	    ZeroGS::Flush(0);
	    ZeroGS::Flush(1);
	    ZeroGS::vb[0].fba.fba = fba;
    }
}
