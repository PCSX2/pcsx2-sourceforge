...
void TransferHostLocal##psm(const void* pbyMem, u32 nQWordSize) \
{ \
	START_HOSTLOCAL(); \
	\
	const T* pbuf = (const T*)pbyMem; \
	int endY, alignedY, alignedX; \
	int nSize = nQWordSize*4*2/TRANSMIT_PITCH##TransSfx(2, T); \
	int pitch, area, fracX; \
	bool bAligned, bCanAlign; \
	nSize = min(nSize, gs.imageWnew * gs.imageHnew); \
	\
	endY = ROUND_UPPOW2(i, blockheight); \
	alignedY = ROUND_DOWNPOW2(gs.imageEndY, blockheight); \
	alignedX = ROUND_DOWNPOW2(gs.imageEndX, blockwidth); \
	bCanAlign = MOD_POW2(gs.trxpos.dx, blockwidth) == 0 && (j == gs.trxpos.dx) && (alignedY > endY) && alignedX > gs.trxpos.dx; \
	\
	if( (gs.imageEndX-gs.trxpos.dx)%widthlimit ) { \
		/* hack */ \
	...
	bAligned = !((u32)(uptr)pbuf & 0xf) && (TRANSMIT_PITCH##TransSfx(pitch, T)&0xf) == 0; \
	\
