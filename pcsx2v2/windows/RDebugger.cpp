...
DWORD WINAPI ServingFunction(LPVOID lpParam){
	static char	buffer[BUFFERSIZE],				//a big buffer
	...
	if ((remote = accept(serversocket, (struct sockaddr*)&saClient, (int*)&size))==INVALID_SOCKET){
	...
	while (!exit && readData((char*)buffer)){
	...
	while (1){
		if (runStatus==RUN){
			if (PSMu32(cpuRegs.pc)==0x0000000D){
				...
				InterlockedExchange((LONG*)&runStatus, STOP);
				continue;
			}
			if ((runCode==2) && (//next
