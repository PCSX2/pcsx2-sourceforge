...
int sioInit() {
	...
	sio.packetsize = 0;
	sio.terminator =0x5A; // Command terminator 'U'
...
void SIO_CommandWrite(u8 value,int way) {
	...
	switch (sio.mcdst) {
		case 1:
			sio.packetsize++;
			if (sio.rdwr) { SIO_INT(); sio.parp++; return; }
			sio.parp = 1;
			switch (value) {
			case 0x11: // RESET
...
//				MEMCARDS_LOG("MC(%d) command 0x%02X\n", ((sio.CtrlReg&0x2000)>>13)+1, value);
			...
			case 0xf7: 
				...
				sio.buf[4]=sio.terminator;
				sio.buf[3]='+';
				sio2.packet.recvVal3 = sio.bufcount << 16;
...
//				MEMCARDS_LOG("MC(%d) WRITE command 0x%02X\n\n\n\n\n", ((sio.CtrlReg&0x2000)>>13)+1, value);
				...
				if (sio.parp==sio.bufcount-2) {
					if (xor(&sio.buf[3], sio.bufcount-5)==value) {
                        _SaveMcd((char*)&sio.buf[3], (512+16)*sio.sector+sio.k, sio.bufcount-5);
...
#endif
					_ReadMcd((char*)&sio.buf[4], (512+16)*sio.sector+sio.k, value);
					if(sio.mode==2) 
					{
						int j;
						for(j=0; j < value; j++)
